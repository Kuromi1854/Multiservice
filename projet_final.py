...
# -------------------------------------------------------------------------
# CONFIGURATION ET FONCTIONS DE LA BASE DE DONNÉES SQL
# -------------------------------------------------------------------------
DB_NAME = "koopar.db"
...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        """,
            [("EMP001", "Stanislas", "1234"), ("EMP002", "Djiwana", "abcd")],
        )
    conn.commit()
    conn.close()
...
# -------------------------------------------------------------------------
# Tout ce qui est css pour le look
# -------------------------------------------------------------------------
st.markdown(
    """
    <style>
...
if profil == "Client":
    ...
    with col1:
        ...
        try:
            if service == "Wave":
                st.image("images/wave.png", width=100)
            elif service == "orange money":
                st.image("images/orange_money.jpg", width=200)
            elif service == "Touch Point":
                st.image("images/touch_point.png", width=200)
            elif service == "Wizall":
                st.image("images/wizall.png", width=100)
            elif service == "Djamo":
                st.image("images/djamo.png", width=200)
        except:
    ...
    if not st.session_state.en_cours_de_validation:
        if st.button("Valider la demande", use_container_width=True):
            if len(telephone) == 9 and telephone.startswith("7"):
                ...
                st.success("Code de vérification généré avec succès !")
                time.sleep(1)
                st.rerun()
            else:
        ...
        st.markdown("---")
        st.info(
            f"📱 **[SIMULATION SMS]** Le code secret envoyé sur le numéro {st.session_state.temp_transaction['Client_Tel']} est : **{st.session_state.code_sms_genere}**"
        )

        code_saisi = st.text_input(
            "👉 Entrez le code à 4 chiffres ci-dessus pour confirmer",
            max_chars=4,
            type="password",
            key="input_otp",
        )
        ...
        with col_btn1:
            if st.button("Confirmer et Valider la demande", use_container_width=True):
                if code_saisi == st.session_state.code_sms_genere:
                    ...
                    st.success(
                        f"✅ Transaction {tx_id} enregistrée en Base SQL avec succès !"
                    )
                    st.session_state.en_cours_de_validation = False
                    ...
                    st.error(
                        "❌ Code incorrect. Veuillez recopier le code indiqué juste au-dessus."
                    )
